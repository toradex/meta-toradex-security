...
1. For HAB-based devices, `TDX_IMX_HAB_CST_SRK_CA` must be set to `1` and the SRK certificates must be generated with the CA flag set. Build errors will happen if the variable is set to `0` because the signing logic currently unconditionally tries to use the CSF and IMG certificates which are not generated by CST when the CA flag is not set.
2. For AHAB-based devices, `TDX_IMX_HAB_CST_SRK_CA` must be set to `0` and the SRK certificates must be generated with the CA flag **not** set. This is because the signing logic currently implemented does not support the use of the image signing key (SGK).
